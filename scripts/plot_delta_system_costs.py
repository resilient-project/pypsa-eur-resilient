...
if __name__ == "__main__":
    ...
    run_to_subtract = plotting["run_to_subtract"]
    ...
    for run in runs:
        ...
        df_system_costs[run] = costs

    df_system_costs = pd.concat(df_system_costs, axis=1)
    df_system_costs.fillna(0, inplace=True)

    grouped_system_costs= df_system_costs.groupby(grouping).sum()
    delta_system_costs= grouped_system_costs.sub(grouped_system_costs[run_to_subtract], axis=0)
    ...
    # Normalize and sort
    plot_df = delta_system_costs.sort_values(by="pcipmi", ascending=False).div(norm)
